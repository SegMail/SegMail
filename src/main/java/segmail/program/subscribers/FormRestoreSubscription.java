 ...
 */
@RequestScoped
@Named("FormRestoreSubscription")
public class FormRestoreSubscription {
    ...
    public void restore() {
        try {
            List<Subscription> result = subService.updateSubscription(unsubKey,SUBSCRIPTION_STATUS.CONFIRMED);
            
            FacesMessenger.setFacesMessage(ProgramSubscribers.class.getSimpleName(), FacesMessage.SEVERITY_FATAL, 
                    "Subscription to "+result.get(0).getTARGET().getLIST_NAME()+" has been restored.", "");
