...
public class UserService extends DBService {
		...
		try {
			...
			if (existingUserTypes != null && !existingUserTypes.isEmpty()) {
				throw new UserTypeException(
						"Usertype name \"" + userTypeName + "\" has been taken. Please choose a different name.");
			}
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public User createUser(long userTypeId) throws UserCreationException, DBConnectionException {
		try {
	...
	public List<UserType> getAllUserTypes() throws DBConnectionException {
		...
		try {
			...
			Root<UserType> sourceEntity = criteria.from(UserType.class); // FROM
																			// UserType
			...
			List<UserType> results = em.createQuery(criteria)
					// .setFirstResult(0)
					// .setMaxResults(GlobalValues.MAX_RESULT_SIZE_DB) //Don't
					// do this first, not necessary!
					.getResultList();
	...
	public UserType getUserTypeById(long userTypeId) throws DBConnectionException {
		...
		try {
			...
			criteria.select(sourceEntity); // SELECT *
			criteria.where(builder.equal(sourceEntity.get(UserType_.OBJECTID), userTypeId)); // WHERE
																								// USERTYPENAME
																								// =
																								// userTypeName

			UserType result = em.createQuery(criteria).getSingleResult(); // appropriate!
																			// because
																			// it
																			// is
																			// an
	...
	public List<UserType> getUserTypeByName(String userTypeName) throws DBConnectionException {
		...
		try {
			/*
			 * CriteriaBuilder builder = em.getCriteriaBuilder();
			 * CriteriaQuery<UserType> criteria =
			 * builder.createQuery(UserType.class); Root<UserType> sourceEntity
			 * = criteria.from(UserType.class); //FROM UserType
			 * 
			 * criteria.select(sourceEntity); // SELECT *
			 * criteria.where(builder.equal(sourceEntity.get(UserType_.
			 * USERTYPENAME), userTypeName)); //WHERE USERTYPENAME =
			 * userTypeName
			 * 
			 * List<UserType> results = em.createQuery(criteria)
			 * .getResultList();
			 */
			List<UserType> results = this.genericEnterpriseObjectService.getEnterpriseObjectsByName(userTypeName,
					UserType.class);
	...
	public User getUserById(long userId) throws DBConnectionException {
		try {
			 ...
			 * Root<User> sourceEntity = criteria.from(User.class); //FROM
			 * UserType
			 * 
			 * criteria.select(sourceEntity); // SELECT *
			 * criteria.where(builder.equal(sourceEntity.get(User_.OBJECTID),
			 * userId)); //WHERE USERTYPENAME = userTypeName
			 * 
	...
	public void login(String username, String password, Map<String, Object> userValues)
			throws UserAccountLockedException, UserLoginException, DBConnectionException {
		try {
			// Check if username exists
			UserAccount userAccount = this.getUserAccountByUsername(username);
			if (userAccount == null) // Do not tell user that username does not
										// exist for security reasons
				throw new UserLoginException("Wrong credentials.");

			// If user's account is already locked, no need to authenticate
			// further
			...
			if (!secureHash.equals(userAccount.getPASSWORD())) { // authentication
																	// fails
			...
			if (secureHash.equals(userAccount.getPASSWORD())) {
				// Only if there were any unsuccessful login attempts, reset
				// counter
				...
				/*
				 * userValues.put(USER, user); userValues.put(USER_TYPE,
				 * user.getUSERTYPE()); userValues.put(USER_ATTRIBUTES,
				 * preferences);
				 */
	...
	public List<UserPreferenceSet> getUserPreferences(long userid) {
		...
		Root<UserPreferenceSet> sourceEntity = criteria.from(UserPreferenceSet.class); // FROM
																						// UserAccount
		...
		/*
		 * Faulty assumption to use getSingleResult() getSingleResult() throws
		 * exception as long as there is no 1 single result returned!
		 *
		 * UserPreferenceSet result = em.createQuery(criteria)
		 * .getSingleResult();
		 */

		List<UserPreferenceSet> results = em.createQuery(criteria).getResultList();

		return results;
	}

	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public UserAccount getUserAccountByUsername(String username) throws DBConnectionException {
		try {
			CriteriaBuilder builder = em.getCriteriaBuilder();
			CriteriaQuery<UserAccount> criteria = builder.createQuery(UserAccount.class);
			Root<UserAccount> sourceEntity = criteria.from(UserAccount.class); // FROM
																				// UserType

			criteria.select(sourceEntity); // SELECT *
			criteria.where(builder.equal(sourceEntity.get(UserAccount_.USERNAME), username)); // WHERE
																								// USERTYPENAME
																								// =
																								// userTypeName

			// Temporary measure before we find a better way to define the
			// underlying
			// data of UserAccount object and subsequently how to retrieve the
			// correct
			// result.
			//
			List<UserAccount> results = em.createQuery(criteria).getResultList();
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public UserAccount getUserAccountById(long userid) throws DBConnectionException {
		try {
			CriteriaBuilder builder = em.getCriteriaBuilder();
			CriteriaQuery<UserAccount> criteria = builder.createQuery(UserAccount.class);
			Root<UserAccount> sourceEntity = criteria.from(UserAccount.class); // FROM
			...
			criteria.select(sourceEntity); // SELECT *
			criteria.where(builder.equal(sourceEntity.get(UserAccount_.OWNER), userid)); // WHERE
																							// USERTYPENAME
																							// =
																							// userTypeName
	...
	public String getUserProfilePicLocation(long userid) throws DBConnectionException {
		...
		try {
			...
			criteria.select(sourceEntity.get(UserAccount_.PROFILE_PIC_URL)); // SELECT
																				// PROFILE_PIC_URL
			criteria.where(builder.equal(sourceEntity.get(UserAccount_.OWNER), userid)); // WHERE
																							// OWNER.OBJECT_ID
																							// =
			...
			List<String> results = em.createQuery(criteria).getResultList();
	...
	@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
	public APIAccount getAPIAccountById(long userid) throws DBConnectionException {
		try {
			CriteriaBuilder builder = em.getCriteriaBuilder();
			CriteriaQuery<APIAccount> criteria = builder.createQuery(APIAccount.class);
			Root<APIAccount> sourceEntity = criteria.from(APIAccount.class); // FROM
																				// UserType

			criteria.select(sourceEntity); // SELECT *
			criteria.where(builder.equal(sourceEntity.get(APIAccount_.OWNER), userid)); // WHERE
																						// USERTYPENAME
																						// =
																						// userTypeName
			...
			// result.
			//
			List<APIAccount> results = em.createQuery(criteria).getResultList();
